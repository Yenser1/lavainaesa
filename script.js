...
document.addEventListener("keydown", (event) => {
    if (event.key.toLowerCase() === "z") { // Detecta 'z' o 'Z' de manera case-insensitive
            ...
            function typeChar() {
                if (i < content.length) {
                    ...
                    if (currentChar === "<" && content.substring(i, i + 5) === "<span") {
                        ...
                        let delay = 1; // Tiempo normal
                        if ([",", ".", "-"].includes(currentChar)) {
                            delay = 1; // Pausa más larga en esos caracteres
                        }
                    ...
                    setTimeout(typeChar, 1); // Velocidad de escritura normal
                } else {
                    paragraphIndex++;
